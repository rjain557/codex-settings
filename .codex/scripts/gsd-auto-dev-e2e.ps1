...
"@
    ...
    $resolvedStatusPath = if ([System.IO.Path]::IsPathRooted($StatusFile)) {
        [System.IO.Path]::GetFullPath($StatusFile)
    } else {
        [System.IO.Path]::GetFullPath((Join-Path $ProjectRoot $StatusFile))
    }
    ...
    Write-Host ("Monitor active in this chat every {0}s (status: {1})" -f $monitorIntervalSeconds, $resolvedStatusPath) -ForegroundColor DarkCyan
    ...
    while (-not $proc.HasExited) {
        if (((Get-Date) - $lastMonitorAt).TotalSeconds -ge $monitorIntervalSeconds) {
            ...
            if ([string]::IsNullOrWhiteSpace($latest)) {
                ...
                Write-Host ("[{0}] monitor {1}" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $latest) -ForegroundColor DarkCyan
            }
    ...
    if ([string]::IsNullOrWhiteSpace($finalStatus)) {
        ...
        Write-Host ("Worker finished (PID={0}, exit={1}). Last status: {2}" -f $proc.Id, $proc.ExitCode, $finalStatus) -ForegroundColor Yellow
    }
